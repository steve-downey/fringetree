...
TEST(TreeTest, TestGTest) { ASSERT_EQ(1, 1); }
...
TEST(TreeTest, breadth) {
    using Tree = Tree<int, int>;
    auto t     = Tree::branch(Tree::branch(Tree::leaf(1), Tree::leaf(2)),
                          Tree::leaf(3));
    ...
    auto t2 = Tree::branch(Tree::branch(Tree::empty(), Tree::leaf(1)),
...
TEST(TreeTest, prepend) {
    ...
    auto t3  = Tree::branch(Tree::branch(Tree::empty(), Tree::leaf(1)),
                           Tree::branch(Tree::leaf(2), Tree::empty()));
...
TEST(TreeTest, leftView) {
    ...
    auto left   = Tree::branch(Tree::branch(Tree::leaf(1), Tree::leaf(2)),
                             Tree::leaf(3));
...
TEST(TreeTest, concat) {
    ...
    auto right = Tree::branch(Tree::branch(Tree::leaf(1), Tree::leaf(2)),
                              Tree::leaf(3));
...
TEST(TreeTest, measure) {
    ...
    auto tree = Tree::branch(branch1, leaf3);
...
struct Min {
    using value_type = N;
    value_type  value_;
    static auto min() -> Min { return std::numeric_limits<N>::min(); }

    Min() : value_(min().value_) {}
    Min(value_type v) : value_(v) {}

    friend bool operator==(const Min& lhs, const Min& rhs) {
...
template <>
constexpr inline auto monoid::monoid_concept_map<Min<int>> =
    MinMonoidMap<int>{};
...
TEST(TreeTest, MinTree) {
    ...
    auto tree = Tree::branch(branch1, leaf1);
